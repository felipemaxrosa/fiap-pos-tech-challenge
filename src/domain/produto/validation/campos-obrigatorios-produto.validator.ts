...
@Injectable()
export class CamposObrigatoriosProdutoValidator
  implements SalvarProdutoValidator
{
  ...
  async validate(produto: Produto): Promise<boolean> {
    ...
    if (
      !CamposObrigatoriosProdutoValidator.VALID_ID_CATEGORIA_PRODUTO.includes(
        produto.idCategoriaProduto,
      )
    ) {
      throw new ValidationException(
