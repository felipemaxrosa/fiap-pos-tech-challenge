...
export class CpfUnicoClienteValidator implements SalvarClienteValidator {
  public static CPF_UNICO_CLIENTE_VALIDATOR_ERROR_MESSAGE =
    'O cpf do cliente não é único';
  ...
  async validate(cliente: Cliente): Promise<boolean> {
    this.logger.log(
      `Inicializando validação ${CpfUnicoClienteValidator.name} para salvar o cliente: ${cliente.cpf}`,
    );

    return this.repository.findBy({ cpf: cliente.cpf }).then((clients) => {
      if (clients.length > 0) {
        this.logger.error(
          `Cliente já existente na base de dados com o CPF: ${cliente.cpf}`,
        );
      ...
      this.logger.debug(
        `${CpfUnicoClienteValidator.name} finalizado com sucesso para cliente: ${cliente.cpf}`,
      );
      return true;
    });
  }
