...
describe('CheckoutPedidoValidator', () => {
   let validator: CheckoutPedidoValidator;
   ...
   beforeEach(async () => {
      ...
      validator = module.get<CheckoutPedidoValidator>(CheckoutPedidoValidator);
   });
   ...
   describe('validate', () => {
      ...
      it('não deve validar pedido quando id estiver vazio', async () => {
         await expect(validator.validate(pedidoSemId)).rejects.toThrowError(
            CheckoutPedidoValidator.PEDIDO_INEXISTENTE_ERROR_MESSAGE,
         );
      });
