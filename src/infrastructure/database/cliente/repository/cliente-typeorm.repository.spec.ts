...
describe('ClienteTypeormRepository', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        //  IRepository<Cliente> provider
        {
          ...
          useFactory: (
            repositoryTypeOrm: Repository<ClienteEntity>,
          ): IRepository<Cliente> => {
    ...
    repositoryTypeOrm = module.get<Repository<ClienteEntity>>(
  ...
  describe('save', () => {
    ...
    it('não deve salvar cliente quando houver um erro de banco ', async () => {
      ...
      await expect(repository.save(cliente)).rejects.toThrowError(
        RepositoryException,
      );
    });
  });
  ...
  describe('findBy', () => {
    it('deve buscar cliente pelo cpf', async () => {
      repositoryTypeOrm.findBy = jest.fn().mockImplementation((attributes) => {
        return Promise.resolve(
          attributes['cpf'] === cliente.cpf ? [clienteEntity] : {},
        );
      });

      await repository
        .findBy({ cpf: cliente.cpf })
    ...
    it('deve buscar cliente pelo email', async () => {
      repositoryTypeOrm.findBy = jest.fn().mockImplementation((attributes) => {
        return Promise.resolve(
          attributes['email'] === cliente.email ? [clienteEntity] : {},
        );
      });

      await repository
        .findBy({ email: cliente.email })
        .then((clientesEncontrados) => {
          // verifica se o cliente salvo contém os mesmos dados passados como input
          clientesEncontrados.forEach((cliente) => {
    ...
    it('deve buscar cliente pelo nome', async () => {
      repositoryTypeOrm.findBy = jest.fn().mockImplementation((attributes) => {
        return Promise.resolve(
          attributes['nome'] === cliente.nome ? [clienteEntity] : {},
        );
      });

      await repository
        .findBy({ nome: cliente.nome })
        .then((clientesEncontrados) => {
          // verifica se o cliente salvo contém os mesmos dados passados como input
          clientesEncontrados.forEach((cliente) => {
    ...
    it('deve buscar cliente pelo id', async () => {
      repositoryTypeOrm.findBy = jest.fn().mockImplementation((attributes) => {
        return Promise.resolve(
          attributes['id'] === cliente.id ? [clienteEntity] : {},
        );
      });

      await repository
        .findBy({ id: cliente.id })
        .then((clientesEncontrados) => {
          // verifica se o cliente salvo contém os mesmos dados passados como input
          clientesEncontrados.forEach((cliente) => {
    ...
    it('não deve salvar cliente quando houver um erro de banco ', async () => {
      ...
      // verifiaca se foi lançada uma exception na camada infra
      await expect(repository.findBy({})).rejects.toThrowError(
        RepositoryException,
      );
    });
  });
